...
def plot(training_errors, validation_errors):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Error')
    plt.title('Annealing: Training and Testing errors')
...
class MLP:
    ...
    def __init__(self, no_hidden_nodes, annealing_start_param, annealing_end_param):
        ...
        self.learning_parameter = annealing_start_param
        self.annealing_start_param = annealing_start_param
        self.annealing_end_param = annealing_end_param
    ...
    def train(self, no_epochs, input_nodes):
        ...
        for x in range(no_epochs):
            ...
            self.learning_parameter = (
                self.annealing_end_param + (self.annealing_start_param - self.annealing_end_param) *
                (
                    1 - (
                        1/(
                            1+math.exp(10-(20*x)/no_epochs)
                          )
                        )
                )
                                       )
